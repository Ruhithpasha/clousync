...
const ImageUploader = () => {
  ...
  const handleImageError = (imageId) => {
    console.log(`[DEBUG] Image load failed in main gallery: ${imageId}`);
    setBrokenImageIds((prev) => new Set(prev).add(imageId));
  };

  const handleRestoreSuccess = (updatedImage) => {
    console.log(`[DEBUG] Restore success in main gallery:`, updatedImage);
        ...
        if (!response.ok) {
          ...
          if (contentType && contentType.includes("application/json")) {
            ...
            if (response.status === 413) {
              errorMessage =
                "File is too large for the server. Please check Nginx 'client_max_body_size' configuration.";
            } else if (response.status === 404) {
              errorMessage =
                "Upload endpoint not found. Please check your API configuration.";
            } else {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/search-images?query=${encodeURIComponent(query)}`,
        {
          headers: {
            Authorization: `Bearer ${session.access_token}`,
          },
  ...
  const filteredImages = galleryImages.filter(
    (img) =>
      ...
      img.tags?.includes(selectedCategory.toUpperCase()),
  );
      ...
      <div className="flex flex-wrap gap-6 items-center justify-between mb-8">
        ...
        <div className="flex gap-4">
          <div className="bg-white dark:bg-[#1e293b] px-6 py-4 rounded-[32px] shadow-soft border border-[#000B2B]/5 dark:border-white/5">
            <p className="text-[#000B2B]/40 dark:text-white/40 text-xs font-bold uppercase tracking-widest mb-1">
              Total Photos
            </p>
            <p className="text-3xl font-[800] text-[#000B2B] dark:text-white">
              {galleryImages.length}
            </p>
          </div>
          <div className="bg-[#FFC107] px-6 py-4 rounded-[32px] shadow-soft border border-[#000B2B]/5 dark:border-white/5">
            <p className="text-[#000B2B]/40 text-xs font-bold uppercase tracking-widest mb-1">
              Albums
            </p>
            <p className="text-3xl font-[800] text-[#000B2B]">
              {albums.length}
            </p>
          </div>
        </div>
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center w-full">
              <div className="flex -space-x-8 md:-space-x-12 overflow-hidden py-4 shrink-0">
                {previewUrls.slice(0, 3).map((url, i) => (
                  <div
                    key={i}
                    className="w-20 h-20 md:w-32 md:h-32 rounded-2xl md:rounded-3xl overflow-hidden border-2 md:border-4 border-white dark:border-[#1e293b] shadow-xl rotate-[-5deg] first:rotate-0 last:rotate-[5deg] transition-transform hover:rotate-0 hover:z-50 bg-[#F7F7F7] dark:bg-[#0f172a]"
                  >
                    <img
                      src={url}
                      alt="Preview"
              ...
              <div className="flex-grow space-y-4 w-full">
                ...
                <div className="flex flex-wrap gap-4 items-end">
                  <div className="flex-grow max-sm:w-full">
                    <label className="block text-[10px] font-extrabold text-[#000B2B]/40 dark:text-white/40 uppercase tracking-widest mb-2 px-1">
                      Assign Batch to Album
                    </label>
                    <select
                  ...
                  >
                    {uploading ? (
                      <>
                        ...
                        AI Syncing ({uploadProgress.current}/
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className="aspect-[4/5] bg-white dark:bg-[#1e293b] rounded-[40px] animate-pulse"
            />
          ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 bg-white dark:bg-[#1e293b] rounded-[60px] border-2 border-dashed border-[#000B2B]/5 dark:border-white/5">
          ...
          <h3 className="text-2xl font-extrabold text-[#000B2B] dark:text-white mb-2 text-center">
            No {selectedCategory} here yet
          </h3>
          <p className="text-[#000B2B]/40 dark:text-white/40 font-bold text-center">
            AI hasn't tagged any photos with this category.
          </p>
        </div>
      ) : viewMode === "timeline" ? (
          ...
          {Object.keys(groupedImages).map((dateStr) => (
            <div key={dateStr} className="space-y-6">
              <div className="flex items-center gap-4">
                <h3 className="text-xl font-black text-[#000B2B] dark:text-white">
                  {dateStr}
                </h3>
                <div className="h-[1px] bg-[#000B2B]/5 dark:bg-white/10 flex-grow" />
                <span className="text-xs font-bold text-[#000B2B]/20 dark:text-white/20 uppercase tracking-widest">
                  {groupedImages[dateStr].length} items
                </span>
              </div>
                ...
                {groupedImages[dateStr].map((image) => (
                  ...
                  >
                    {brokenImageIds.has(image.id) ? (
                      <div className="w-full h-full flex flex-col items-center justify-center bg-red-50 dark:bg-red-900/10 p-2">
                        <ImageIcon className="text-red-300 mb-1" size={16} />
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setRestoreTarget(image);
                          }}
                          className="text-[8px] font-black bg-red-500 text-white px-2 py-1 rounded-full uppercase"
                        >
                          Restore
                        </button>
                      </div>
          ...
          {filteredImages.map((image, idx) => (
            ...
            >
              ...
              <div className="aspect-[4/5] w-full h-full">
                {brokenImageIds.has(image.id) ? (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-red-50 dark:bg-red-900/20 p-8 text-center space-y-4">
                    <div className="w-16 h-16 bg-red-100 dark:bg-red-900/30 rounded-3xl flex items-center justify-center">
                    ...
                    <div>
                      ...
                      <p className="text-[#000B2B]/40 dark:text-white/40 text-xs font-bold truncate max-w-[150px] mx-auto">
                        {image.original_name}
                      </p>
                    </div>
                    <button
                      ...
                      className="bg-red-500 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-red-500/30 hover:scale-105 active:scale-95 transition-all flex items-center gap-2"
                    >
                      <RefreshCw size={14} />
                      Restore Now
                    </button>
                    ...
                    {image.tags && image.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {image.tags.slice(0, 5).map((tag, i) => (
                          <span
                            key={i}
                            className="px-2 py-1 bg-[#FFC107] rounded-full text-[9px] font-black text-[#000B2B] shadow-lg border border-white/20"
                          >
                            #{tag.toUpperCase()}
                          </span>
                        ))}
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="relative group/album">
                      <select
                        ...
                        disabled={isUpdating === image.id}
                        onChange={(e) =>
                          handleMoveToAlbum(image.id, e.target.value)
                        }
                      >
                        <option value="" className="text-[#000B2B]">
                          Default Library
                        </option>
                        {albums.map((alb) => (
                          <option
              ...
              <div className="absolute top-6 right-6 z-20">
                <div className="bg-white/80 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-sm">
                  ...
                  <span className="text-[10px] font-extrabold text-[#000B2B] uppercase tracking-wider">
                    Sync'd
                  </span>
                </div>
              </div>
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm font-bold text-[#000B2B]/40 dark:text-white/40">
                  This will permanently remove this image from your cloud vault.
                </p>
              </div>
              <div className="flex gap-4">
                <button
